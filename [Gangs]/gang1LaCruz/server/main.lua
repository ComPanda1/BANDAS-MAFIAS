if Config.EnableESXService then
	if Config.MaxInService ~= -1 then
		TriggerEvent('esx_service:activateService', 'gang1', Config.MaxInService)
	end
end

TriggerEvent('esx_phone:registerNumber', 'gang1', _U('alert_gang1'), true, true)
TriggerEvent('esx_society:registerSociety', 'gang1', 'gang1', 'society_gang1', 'society_gang1', 'society_gang1', {type = 'public'})

RegisterNetEvent('esx_gang1:confiscatePlayerItem')
AddEventHandler('esx_gang1:confiscatePlayerItem', function(target, itemType, itemName, amount)
	local source = source
	local sourceXPlayer = ESX.GetPlayerFromId(source)
	local targetXPlayer = ESX.GetPlayerFromId(target)

	if sourceXPlayer.job.name ~= 'gang1' then
		print(('esx_gang1: %s attempted to confiscate!'):format(sourceXPlayer.identifier))
		return
	end
...
AddEventHandler('esx_gang1:handcuff', function(target)
	...
	if xPlayer.job.name == 'gang1' then
		TriggerClientEvent('esx_gang1:handcuff', target)
	else
		print(('esx_gang1: %s attempted to handcuff a player (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_gang1:drag', function(target)
	...
	if xPlayer.job.name == 'gang1' then
		TriggerClientEvent('esx_gang1:drag', target, source)
	else
		print(('esx_gang1: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_gang1:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'gang1' then
		...
		print(('esx_gang1: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_gang1:OutVehicle', function(target)
	...
	if xPlayer.job.name == 'gang1' then
		...
		print(('esx_gang1: %s attempted to drag out from vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)

RegisterNetEvent('esx_gang1:getStockItem')
...
RegisterNetEvent('esx_gang1:putStockItems')
...
ESX.RegisterServerCallback('esx_gang1:getOtherPlayerData', function(source, cb, target, notify)
...
ESX.RegisterServerCallback('esx_gang1:getFineList', function(source, cb, category)
...
ESX.RegisterServerCallback('esx_gang1:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_gang1:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_gang1: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_gang1: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_gang1:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_gang1: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
...
ESX.RegisterServerCallback('esx_gang1:storeNearbyVehicle', function(source, cb, plates)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('playerDropped', function()
	...
	if playerId then
		...
		if xPlayer and xPlayer.job.name == 'gang1' then
			Wait(5000)
			TriggerClientEvent('esx_gang1:updateBlip', -1)
		end
	end
end)

RegisterNetEvent('esx_gang1:spawned')
AddEventHandler('esx_gang1:spawned', function()
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(playerId)
...
RegisterNetEvent('esx_gang1:forceBlip')
AddEventHandler('esx_gang1:forceBlip', function()
	for _, xPlayer in pairs(ESX.GetExtendedPlayers('job', 'gang1')) do
		TriggerClientEvent('esx_gang1:updateBlip', xPlayer.source)
	end
end)
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_phone:removeNumber', 'gang1')
	end
end)
